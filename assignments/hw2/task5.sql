...
SELECT customer_id, name, SUM(product_price) as total_spent
FROM Customer NATURAL JOIN Sales NATURAL JOIN Product
...
-- -- select customers who have 3 distict purchases
-- SELECT customer_id
...
SELECT customer_id, name, COUNT(DISTINCT product_id) as product_count
FROM Customer NATURAL JOIN Sales
WHERE customer_id = SOME(
    SELECT customer_id
    FROM Customer NATURAL JOIN Sales
    GROUP BY customer_id
    HAVING COUNT(DISTINCT product_id) > 3
...
SELECT sales_id, name as customer_name, product_name, sale_date
FROM Sales NATURAL JOIN Product NATURAL JOIN Customer
...
SELECT city, COUNT(customer_id) as total_customers, SUM(product_price) as total_revenue
FROM Customer NATURAL JOIN Sales NATURAL JOIN Product
GROUP BY city
...
SELECT DISTINCT customer_id, name, product_name
FROM Customer NATURAL JOIN Sales NATURAL JOIN Product
